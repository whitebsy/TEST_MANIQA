...
class MANIQA(nn.Module):
    def __init__(self, embed_dim = 768, num_outputs = 1, patch_size = 16, drop = 0.1, # dim需要改变
        ...
        self.conv1 = nn.Conv2d(self.feature_num * 3, self.out1_feature, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        # x = self.dropout(x)   # drop solve over-fitting

        # stage 1
        # x: [b, 196, 768 * 5 * 3] --> [b, 768 * 3, 14, 14]
        ...
        # stage2   进行下采样
        #x: [b, 512, 14, 14] --> [b, 1024, 7, 7]
        x = self.patch_merging(x)

        # x: [b, 1024, 7, 7]  -->  [b, 256, 7, 7]  --> [b, 49, 256]
